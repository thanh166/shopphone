...
<head>
    ...
    <?php require_once ("BackEnd/ConnectionDB/DB_driver.php"); ?>
</head>
...
$result = mysqli_query($db1->__conn,$sql);
...
$db2 = new DB_driver();
$db2-> connect();
$sql1 = "SELECT MONTH(NgayLap) as thang, YEAR(NgayLap) as nam, SUM(TongTien) as tongtien  FROM `hoadon` GROUP by MONTH(NgayLap) ORDER BY NgayLap";
$result1 = mysqli_query($db2->__conn,$sql1);
    ...
    data: {
      labels: [
        <?php
                ...
                {
                    echo '"'.$key1['thang'].', '.$key1['nam'].'",';
                }
        ?>
      ],
      datasets: [
        {
          label: "Doanh thu theo từng tháng (VNĐ)",
          data: [ <?php
                foreach($data1 as $key1)
                {
                    echo '"'.$key1['tongtien'].'",';
