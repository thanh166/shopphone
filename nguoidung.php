...
<body>
	...
	if (isset($_SESSION['currentUser'])) {
		...
		$sqltt = "SELECT n.Ten, SUM(h.TongTien) as tongtien from nguoidung as n, hoadon as h WHERE n.MaND = h.MaND and h.MaND =$manguoidung";
		$sqlsl = "select n.Ten, SUM(c.SoLuong) as soluong from nguoidung as n, hoadon as h, chitiethoadon as c WHERE n.MaND = h.MaND AND h.MaHD = c.MaHD AND h.MaND = $manguoidung";
        $dsdh=(new DB_driver())->get_list($sql);
		$tt = (new DB_driver())->get_list($sqltt);
		$sl = (new DB_driver())->get_list($sqlsl);
		if (sizeof($tt)>0)
			foreach($tt as $data)
			{
				$tongTienTatCaDonHang += $data['tongtien'];	
			}
		else
			$tongTienTatCaDonHang = 1;
		
			if (sizeof($tt)>0)
			foreach($sl as $data1)
			{
				$tongSanPhamTatCaDonHang += $data1['soluong'];	
			}
		else
			$tongSanPhamTatCaDonHang = 1;
        ...
        if (sizeof($dsdh)>0)
        {
            ...
            <table>
                ...
                <tr>
                    <td>Tên: </td>
                    <td> <input type="text" value="'.$row['Ten'].' " readonly> </td>
            ...
            " readonly> </td>
                ...
                <tr>
                    ...
                    <td> <input type="text" value="'.$tongTienTatCaDonHang.'
            ₫" readonly> </td>
                    <td></td>
                </tr>
                <tr>
